...
- **トークン**: 色 `--hirot-green-05..70` / `--hirot-shade-20..70` / `--hirot-ink` / `--hirot-white` / `--hirot-hairline`、面 `--hirot-surface-wash`(green-05) / `--hirot-surface-band`(green-10)。角丸 `--hirot-radius-card: 12px` / `--hirot-radius-panel: 20px`、ピル 9999px。影 `--hirot-halo`。
- **タイポ**: 見出し Hanken Grotesk 300、eyebrow 12px/500/.18em/uppercase/green-60、本文 Noto Sans JP 14〜16px、補足 shade-50。
- **既存パターン**: 導入バンド = `hirot-collection-header`（surface-wash / radius-panel / 56px 48px padding / 44px 見出し）。常時 halo のカード = `card--card` 相当。テキストリンク = `.hirot-staff-card__social`（13px/600/ヘアライン下線）。CTA バンド = newsletter 上書き様式（surface-wash パネル + accent ピル）。
...
```
1. 導入バンド（hirot-collection-header 様式そのまま）
...
## 2. カード `.hirot-flowcard`

| 要素 | 値 | トークン |
|---|---|---|
| コンテナ | 白 / radius-card / **halo 常時**（card--card と同じ「置かれた紙」） / padding 28px。クリック対象ではないので hover リフトなし | `--hirot-white` `--hirot-radius-card` `--hirot-halo` |
| カテゴリーチップ | ピル / green-10 背景 / green-70 文字 / 11px / 500 / .05em（`.hirot-card__badge` の配色反転版） | `--hirot-green-10` `--hirot-green-70` |
| タイトル | 20px / 500 / ink / 1.5（`.hirot-jcard__title` と同値） | `--hirot-ink` |
| 本文 | 14px / 1.8 / shade-50 | `--hirot-shade-50` |
| 図解 | インライン SVG（下記）。width: 100% で可変 | — |
| 体験導線リンク | 13px / 600 / ink / ヘアライン下線 → hover で ink 下線（`.hirot-staff-card__social` と同一様式）+「→」 | `--hirot-hairline` `--hirot-ink` |
...
| 部位 | 形 | 塗り/線 | 文字 |
|---|---|---|---|
| キャプション（トリガー/条件/アクション） | 各ノード上部中央 | — | 10px / .1em / `--hirot-shade-50` |
| ノード共通 | rect 168x48 / rx 12（radius-card と同値） | — | 12.5px、中央揃え、長文は 2 行 |
| トリガー | 同上 | fill `--hirot-green-10` / stroke `--hirot-green-30` | `--hirot-green-70` |
| 条件 | 同上 | fill `--hirot-white` / stroke `--hirot-shade-30` | `--hirot-ink` |
| アクション | 同上 | fill `--hirot-ink` | `--hirot-white`（結論を最も強く） |
| 矢印 | 20px の線 + シェブロン / stroke 1.5（trust-strip の線幅） | `--hirot-shade-40` | — |
...
- 導入バンド: hirot-collection-header と同じ縮小（60em: padding 40px 28px / 34px 見出し、40em: 32px 20px / 28px）。
- SVG は width 100% で等比縮小のみ。1 列時は最大幅いっぱいで十分読める（実測 12.5px x 0.9 倍程度）。
- 体験導線リンクは行高 44px 相当の padding を確保。

## 6. generator への注意点

- 雛形: 導入バンド = `sections/hirot-collection-header.liquid`（ただし page テンプレート用なので `collection.title` でなく section settings の text）。SVG インライン化の作法 = `sections/hirot-trust-strip.liquid`。締め CTA = newsletter 上書き様式を独自 class で再現。
- JS ゼロ（表示系。spec-v3 の二層ルール）。CSS は `assets/hirot.css` 末尾へ。
...
- page.automation テンプレート JSON: hirot-collection-header 系バンド → flow showcase → （任意で hirot-trust-strip）。
