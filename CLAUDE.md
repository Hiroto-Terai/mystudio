...
Shopify テーマ。**Dawn 15.5.0 ベース + 「hirot」ブランドのリスキン/独自セクション**。フリーランスのポートフォリオ用デモストア（`mystudio`）。汎用コーディング規約はグローバル `~/.claude/CLAUDE.md` に従い、ここには本リポジトリ固有のことだけ書く。

構造・命名規則・CSS トークンの網羅的な詳細は **[theme-profile.md](theme-profile.md)**（作業前に一読）。

## アーキテクチャ（コードから読み取りにくい前提）

- カスタム層 = **hirot**。独自セクションは `sections/hirot-*.liquid` と `sections/staff-list.liquid`。それ以外（`main-*`, `cart-*`, ほとんどの `snippets/`・`assets/*.js`）は **Dawn 標準＝原則いじらない**。
- **CSS は単一 `assets/hirot.css` に集約**。`layout/theme.liquid` L289 で `base.css` の後に読み込み Dawn を上書きする。新セクションのスタイルもここに追記する（セクション個別の `{% stylesheet %}` は使わない）。
- hirot セクションは **JS ゼロ・スニペット依存ゼロ**（自己完結）。共有は CSS ユーティリティ class（`.hirot-section-eyebrow` / `.hirot-section-heading` / `.hirot-btn*`）と `--hirot-*` トークン。

## 新しいカスタムセクションを作るとき

- 既存の `sections/hirot-*.liquid` を雛形にして書式を合わせる（我流にしない）。
- クラスは `hirot-` + BEM。色・余白・フォントは `--hirot-*` トークンを使い生 hex を直書きしない（色スケール `--hirot-green-05..70` / `--hirot-shade-20..70`、フォント `--font-body-family`=Noto Sans JP・`--font-heading-family`=Hanken Grotesk）。
