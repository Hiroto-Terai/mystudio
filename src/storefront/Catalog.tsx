// Category list + product detail screens.
...
import type { Catalog as CatalogData, Product } from "./types";
...
export function CatalogList({ data, category, onCategory, onOpen, onAdd, cols = 4 }: CatalogListProps) {
...
export function ProductDetail({ product, data, onAdd, onOpen, cols = 4 }: ProductDetailProps) {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 24px 64px" }}>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 12 }}>
            {gallery.map((g, i) => (
              <button
                key={i}
                onClick={() => setMain(i)}
                style={{ width: 72, height: 72, borderRadius: "var(--radius-md)", overflow: "hidden", border: i === main ? "2px solid var(--ink)" : "1px solid var(--border-hairline)", padding: 0, cursor: "pointer", background: "none" }}
              >
      ...
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 24 }}>
        {related.map((p) => (
          <ProductCard key={p.id} {...p} onClick={() => onOpen(p)} onAdd={() => onAdd(p, 1)} />
        ))}
      </div>
    </div>
  );
